...
out_png <- "04_stsb_ks_heatmap_lower_revY.png"
...
K_tbl <- as.data.frame(K) %>% mutate(Bin = rownames(K)) %>% relocate(Bin)
K_tbl_round <- K_tbl
...
p <- ggplot(K_long, aes(x = Bin_col, y = Bin_row, fill = KS)) +
  ...
  scale_y_discrete(limits = rev(present_bins)) +   # 0–1 at TOP, 5 at BOTTOM
  coord_fixed() +
  labs(
    x = "Bin", y = "Bin"
  ) +
